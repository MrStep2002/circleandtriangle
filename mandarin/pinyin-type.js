(function () {
  ...
  const MODES = [
    { id: "mix", name: "Mix everything", emoji: "🎲", sub: `All ${ALL_WORDS.length} words, one at a time` },
  ...
  function startMode(mode) {
    currentMode = mode;
    if (mode.id === "mix") pool = ALL_WORDS;
    else if (mode.id === "weak") pool = MandarinProgress.weakWords(ALL_WORDS, 30);
    else pool = TOPICS.find(topic => topic.id === mode.id).words;
  ...
  function chooseWord() {
    ...
    if (currentMode.id === "mix") {
      ...
      if (weak.length && Math.random() < 0.45) choices = weak;
    }
    let choice;
    let attempts = 0;
    do {
      ...
      attempts++;
    } while (choice.pinyin === lastPinyin && attempts < 10);
    return choice;
  }

  function nextWord() {
    word = chooseWord();
    lastPinyin = word.pinyin;
    dirty = false;
    answered = false;
